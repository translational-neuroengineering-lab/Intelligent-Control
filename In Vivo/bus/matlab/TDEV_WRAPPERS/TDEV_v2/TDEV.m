classdef TDEV < handle
    ...
    %
    %   Temporarily set a tag to 1 and then back to 0:
    %      result = obj.trig(TAGNAME)
    %
    ...
    methods %(Access=private)
        function obj = setup(obj)
            ...
            for nm = obj.DEVICE_NAMES
                obj.FS = cat(2, obj.FS, obj.TD.GetDeviceSF(nm{1}));
                ind1 = ind1 + 1;
            end
        end
        ...
        function obj = set_mode(obj, new_mode)
            ...
            if result == 0
                error('System mode change failed')
                return
            end
            ct = 0; tic;
...
%             end
        ...
        function result = find(obj, attr, device, varargin)
            ...
            elseif strcmpi(attr, 'partag')
                % check if tagname is in PARTAG property and get array size
                target = [device '.' tagname];
                result = obj.TD.GetTargetSize(target);
                if result < 1
