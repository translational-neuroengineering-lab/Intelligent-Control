function [datafit,Amps,freqs,Indx]=fitDBS(data,params,HampelParams,plt)
% fits significant sine waves to data (continuous data).
%
% Usage: [datafit,Amps,freqs,Fval,sig]=fitDBS(data,params,HampelParams,plt)
%
%  Inputs:  
...
[Aout,AoutDBS,f,riIndx] = hampeltestc(data,params,HampelParams);

DBSFreqs=f(riIndx);
%datafitDBS=exp(i*2*pi*(0:N-1)'*useFreqs/Fs)*AoutDBS(AllIndx)'+exp(-i*2*pi*(0:N-1)'*useFreqs/Fs)*conj(AoutDBS(AllIndx)');
%datafitDBS=exp(i*2*pi*(0:N-1)'*f/Fs)*AoutDBS+exp(-i*2*pi*(0:N-1)'*f/Fs)*conj(AoutDBS');
if isempty(f0);
   fmax=findpeaksCHRNX(Fval,sig);
   freqs=cell(1,C);
   Amps=cell(1,C);
   datafit=data;
   for ch=1:C;
